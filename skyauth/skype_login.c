...
u32 encode32(u8 *const to, const u32 *const from, const u32 words) {
  ...
  for (i = 0; i < words; i++) {
    for (a = from[i]; a > 0x7F; a >>= 7, n++) to[n] = (u8) a | 0x80;
    to[n++] = (u8) a;
  }
  return n;
}
...
int Produce_Session_Key(const u32 *rand192, u32 *key256, u32 *encrypted_key1536) {
  ...
  x->len = 48;
  for (i = 0; i < 48; i++) x->w[i] = _bswap32(x192[47 - i]);
  m->len = 48;
  memcpy(m->w, Skype_Login_RSA_Key, 48 * 4);
...
u32 SkypePrepareLoginPackets(u32 *public_key, char *login1, char *login2, u32 *n1, u32 *n2, u32 *key256) {
  ...
  SHA1_hash(public_key,
            128,
            hostid2);    // a = public_key; b = rand192?; c = sha1(ProductId); d = sha1(HDD0 ID); e = sha1(C Volume SN);
  hostid2[3] ^= hostid2[0];
  hostid2[0] = public_key[0];    // matching Skype
  hostid2[2] ^= hostid2[1];
  ...
  p = login1;
  p += attach (p,
               "\x16\x03\x01\x00\xEB\x42\xCD\xEF\xE7\x40\xD7\x2F\x1D\xC0\xC6\x88\x80\xDF\xB7\x75\x37\x18\x69\x62\xB4\xEE\x3E",
               27);
...
int do_skype_login(char *username, char *password) {
  ...
  p = mirvar(0);
  p->len = 16;
  for (i = 0; i < 16; i++) srand32(), p->w[i] = rand32();
  p->w[15] |= 0x80000000;
  nxprime(p, p);
  q = mirvar(0);
  q->len = 16;
  for (i = 0; i < 16; i++) srand32(), q->w[i] = rand32();
  q->w[15] |= 0x80000000;
  nxprime(q, q);
