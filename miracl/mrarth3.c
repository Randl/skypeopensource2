...
void expint(_MIPD_ int b, int n, big x) { /* sets x=b^n */
  unsigned int bit, un;
#ifdef MR_OS_THREADS
...
void power(_MIPD_ big x, long n, big z, big w) { /* raise big number to int power  w=x^n *
  ...
  if (w == z)
    forever { /* "Russian peasant" exponentiation */
      if (n % 2 != 0L)
...
BOOL nroot(_MIPD_ big x, int n, big w) {  /*  extract  lower approximation to nth root   *
  ...
  d = 0;
  p = 1;
  while (!mr_mip->ERNUM) { /* Newtons method */
    ...
    if (full && mr_abs(dif) < n) { /* test for finished */
      while (dif < 0) {
        rem = 0;
