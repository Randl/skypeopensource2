...
static mr_utype twop(int n) { /* 2^n */
#ifdef MR_FP
  int i;
#endif
  mr_utype r = 1;
...
int mr_fft_init(_MIPD_ int logn, big m1, big m2, BOOL cr) { /* logn is number of bits, m1 and m2 are the largest elements
  ...
  pr = 0;
  while (size(mr_mip->w5) > 0) { /* find out how many primes will be needed */
    do {
      kk--;
      p = kk * newn + 1;
...
/* if nothing has changed, don't recalculate */
  ...
  kk = mr_shiftbits((mr_small) 1, MIRACL - 2 - logn);
  if (mr_mip->base != 0) while (4 * kk * newn > mr_mip->base) kk = mr_shiftbits(kk, -1);
  for (i = 0; i < pr; i++) { /* find the primes again */
...
/* set up chinese remainder structure */
  if (cr) if (!scrt_init(_MIPP_ &mr_mip->chin, pr, mr_mip->prime)) fft_reset(_MIPPO_);
  return pr;
}

void fft_reset(_MIPDO_) { /* reclaim any space used by FFT */
  int i;
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif

  if (mr_mip->degree != 0) { /* clear any precomputed tables */
  ...
  if (mr_mip->logN != 0) { /* clear away old stuff */
    for (i = 0; i < mr_mip->nprimes; i++) {
...
void mr_dif_fft(_MIPD_ int logn, int pr, mr_utype *data) { /* decimate-in-frequency fourier transform */
  ...
  for (k = 0; k < logn; k++) {
    ...
    jj = newn / istep;
    ii -= jj;
    for (i = 0; i < newn; i += istep) { /* special case root=1 */
      j = i + mmax;
      temp = data[i] - data[j];
...
void mr_dit_fft(_MIPD_ int logn, int pr, mr_utype *data) { /* decimate-in-time inverse fourier transform */
...
static void modxn_1(_MIPD_ int n, int deg, big *x) {  /* set X (of degree deg) =X mod x^n-1 = X%x^n + X/x^n */
  int i;
  for (i = 0; n + i <= deg; i++) {
...
BOOL mr_poly_rem(_MIPD_ int dg, big *G, big *R) { /* G is a polynomial of degree dg - G is overwritten */
  ...
  for (i = 0; i < np; i++) {
    ...
    if (mr_mip->logN > logn) { /* adjust 1/N log p for N/2, N/4 etc */
...
void mr_polymod_set(_MIPD_ int n, big *rf, big *f) { /* n is degree of f */
...
int mr_ps_big_mul(_MIPD_
                  int deg,
                  big *x,
                  big *y,
                  big *z) { /* Multiply two power series with large integer parameters */
  int i, j, newn, logn, np;
  mr_utype inv, p, fac;
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
...
/* compute coefficients modulo fft primes */
  ...
  for (j = 0; j < deg; j++) {
    ...
    if (mr_compare(z[j], mr_mip->w4) >= 0) { /* In higher half of range, so number is negative */
...
int mr_poly_mul(_MIPD_
                int degx,
                big *x,
                int degy,
                big *y,
                big *z) { /*  Multiply two polynomials. The big arrays are of size degree */
...
int mr_poly_sqr(_MIPD_ int degx, big *x, big *z) { /*  Multiply two polynomials. The big arrays are of size degree */
  int i, j, newn, logn, np, degree;
  mr_utype inv, p, fac;
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
...
/* compute coefficients modulo fft primes */
  for (i = 0; i < np; i++) {
    ...
    inv = mr_mip->inverse[i];
    if (mr_mip->logN > logn) { /* adjust 1/N log p for smaller N  */
      fac = twop(mr_mip->logN - logn);
      inv = smul(fac, inv, p);
    }
    for (j = 0; j <= degree; j++)
  ...
  mr_shift(_MIPP_ mr_mip->modulus, (int) mr_mip->modulus->len, mr_mip->w6);
  /* w6 = N.R */
  for (j = 0; j <= degree; j++) { /* apply CRT to each column */
    for (i = 0; i < np; i++)
      mr_mip->cr[i] = mr_mip->t[i][j];
...
static BOOL init_it(_MIPD_ int logn) { /* find primes, table of roots, inverses etc for new N */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
BOOL fastmultop(_MIPD_ int n, big x, big y, big z) { /* only return top n words... assumes x and y are n in length */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
  int len;
...
void fft_mult(_MIPD_ big x, big y, big z) { /* "fast" O(n.log n) multiplication */
  ...
  for (pr = 0; pr < 3; pr++) { /* multiply mod each prime */
    p = mr_mip->prime[pr];
    ...
    for (i = 0; i < newn; i++) {  /* "multiply" Fourier transforms */
#ifdef MR_FP_ROUNDING
    ...
    for (i = 0; i < newn; i++) {  /* convert to mixed radix form     *
  ...
  c1 = c2 = 0;
  if (mr_mip->base == 0)
    for (i = 0; i < zl; i++) { /* propagate carries */
#ifndef MR_NOFULLWIDTH
      v1 = d0[i];
      v2 = d1[i];
      v3 = d2[i];
