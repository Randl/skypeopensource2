...
BOOL nres_sqroot(_MIPD_ big x, big w) { /* w=sqrt(x) mod p. This depends on p being prime! */
  int t, js;
  ...
  if (jack(_MIPP_ w, mr_mip->modulus) != 1) { /* Jacobi test */
    zero(w);
    MR_OUT
    return FALSE;
  }
  ...
  if (js == 1) { /* 3 mod 4 primes - do a quick and dirty sqrt(x)=x^(p+1)/4 mod p */
    ...
    copy(mr_mip->one, w);
    forever { /* Simple Right-to-Left exponentiation */
...
BOOL sqroot(_MIPD_ big x, big p, big w) { /* w = sqrt(x) mod p */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
  if (mr_mip->ERNUM) return FALSE;
  ...
  if (subdivisible(_MIPP_ p, 2)) { /* p must be odd */
    zero(w);
    MR_OUT
    return FALSE;
  }
