...
static int dquot(_MIPD_ big x, int num) { /* generate c.f. for a double D */
  int m;
#ifdef MR_OS_THREADS
...
void dconv(_MIPD_ double d, flash w) { /* convert double to rounded flash */
  int s;
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
double fdsize(_MIPD_ flash w) { /* express flash number as double. */
  int i, s, en, ed;
