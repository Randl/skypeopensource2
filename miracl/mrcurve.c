...
static void epoint_getrhs(_MIPD_ big x, big y) { /* x and y must be different */
...
BOOL epoint_x(_MIPD_ big x) { /* test if x is associated with a point on the   *
...
BOOL epoint_set(_MIPD_ big x, big y, int cb, epoint *p) { /* initialise a point on active ecurve            *
...
/* find x^3+Ax+B */
  ...
  else { /* no y supplied - calculate one. Find square root */
#ifndef MR_NOSUPPORT_COMPRESSION
...
void epoint_getxyz(_MIPD_ epoint *p, big x, big y, big z) { /* get (x,y,z) coordinates */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
  ...
  if (p->marker == MR_EPOINT_INFINITY) {
    ...
    if (mr_mip->coord == MR_AFFINE) { /* (0,1) or (0,0) = O */
#endif
      if (x != NULL) zero(x);
      if (mr_mip->Bsize == 0) {
...
int epoint_get(_MIPD_ epoint *p, big x, big y) { /* Get point co-ordinates in affine, normal form       *
  ...
  if (!epoint_norm(_MIPP_ p)) { /* not possible ! */
    MR_OUT
    return (-1);
  }
...
BOOL epoint_norm(_MIPD_ epoint *p) { /* normalise a point */
...
BOOL epoint_multi_norm(_MIPD_
                       int m,
                       big *work,
                       epoint **p) { /* Normalise an array of points of length m<MR_MAX_M_T_S - requires a workspace array of length m */
...
void ecurve_multi_add(_MIPD_ int m, epoint **x, epoint **w) { /* adds m points together simultaneously, w[i]+=x[i] */
  int i, *flag;
  big *A, *B, *C;
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
  ...
  MR_IN(122)
#ifndef MR_AFFINE_ONLY
  if (mr_mip->coord == MR_AFFINE) { /* this can be done faster */
#endif
...
void ecurve_double(_MIPD_ epoint *p) { /* double epoint on active ecurve */
...
#ifndef MR_AFFINE_ONLY
  if (mr_mip->coord == MR_AFFINE) { /* 2 sqrs, 1 mul, 1 div */
#endif
...
static BOOL ecurve_padd(_MIPD_ epoint *p, epoint *pa) { /* primitive add two epoints on the active ecurve - pa+=p;   *
...
#endif
#ifndef MR_AFFINE_ONLY
  if (mr_mip->coord == MR_AFFINE) {  /* 1 sqr, 1 mul, 1 div */
#endif
...
int ecurve_add(_MIPD_ epoint *p, epoint *pa) {  /* pa=pa+p; */
...
void epoint_negate(_MIPD_ epoint *p) { /* negate a point */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
  if (mr_mip->ERNUM) return;
...
int ecurve_mult(_MIPD_ big e, epoint *pa, epoint *pt) { /* pt=e*pa; */
...
/*    epoint_norm(_MIPP_ pa); */
  ...
  if (mr_mip->base == mr_mip->base2) {
    ...
    for (i = 1; i < MR_ECC_STORE_N - 1; i++) { /* precomputation */
    ...
    while (size(mr_mip->w11) > 1) { /* add/subtract method */
      if (mr_mip->user != NULL) (*mr_mip->user)();
...
void ecurve_multn(_MIPD_ int n, big *y, epoint **x, epoint *w) { /* pt=e[o]*p[0]+e[1]*p[1]+ .... e[n-1]*p[n-1]   */
  int i, j, k, m, nb, ea;
  epoint **G;
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
...
static BOOL ecurve_add_sub(_MIPD_ epoint *P, epoint *Q, epoint *PP, epoint *PM) {
  ...
  if (mr_compare(P->X, Q->X) == 0) { /* P=Q or P=-Q - shouldn't happen */
    epoint_copy(P, PP);
...
void ecurve_mult2(_MIPD_ big e, epoint *p, big ea, epoint *pa, epoint *pt) { /* pt=e*p+ea*pa; */
  int e1, h1, e2, h2, bb;
  epoint *p1, *p2, *ps[2];
#ifdef MR_STATIC
