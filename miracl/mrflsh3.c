...
static int norm(_MIPD_ int type, flash y) { /* convert y to first quadrant angle *
  ...
  if (fcomp(_MIPP_ y, mr_mip->w8) > 0) { /* reduce mod 2.pi */
  ...
  if (fcomp(_MIPP_ y, mr_mip->pi) > 0) { /* if greater than pi */
    fsub(_MIPP_ y, mr_mip->pi, y);
    if (type != TAN) s = (-s);
  }
  fpmul(_MIPP_ mr_mip->pi, 1, 2, mr_mip->w8);
  if (fcomp(_MIPP_ y, mr_mip->w8) > 0) { /* if greater than pi/2 */
...
static int tan1(_MIPD_ big w, int n) {  /* generator for C.F. of tan(1) */
  if (n == 0) return 1;
  if (n % 2 == 1) return 2 * (n / 2) + 1;
  else return 1;
}

void ftan(_MIPD_ flash x, flash y) { /* calculates y=tan(x) */
  ...
  op[1] = op[3] = 1;
  op[2] = 0;
  for (m = sqrn; m > 1; m--) { /* Unwind C.F for tan(x)=z/(1-z^2/(3-z^2/(5-...)))))) */
    op[4] = 2 * m - 1;
  ...
  op[1] = op[2] = op[3] = 1;
  op[4] = (-1);
  for (i = 0; i < nsq; i++) { /* now square it back up again */
...
void fatan(_MIPD_ flash x, flash y) { /* calculate y=atan(x) */
  int s, c, op[5];
  BOOL inv, hack;
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
  ...
  if (c > 0) { /* atan(x)=pi/2 - atan(1/x) for x>1 */
    inv = TRUE;
    frecip(_MIPP_ y, y);
  }
  hack = FALSE;
  if (mr_lent(y) <= 2) { /* for 'simple' values of y */
    hack = TRUE;
  ...
  while (mr_mip->workprec != mr_mip->nib) { /* Newtons iteration w11=w11+(y-tan(w11))/(tan(w11)^2+1) */
...
void fsin(_MIPD_ flash x, flash y) { /*  calculate y=sin(x) */
  int sgn, op[5];
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
void fasin(_MIPD_ flash x, flash y) { /* calculate y=asin(x) */
  int s, op[5];
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
void fcos(_MIPD_ flash x, flash y) { /* calculate y=cos(x) */
  int sgn, op[5];
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
void facos(_MIPD_ flash x, flash y) { /* calculate y=acos(x) */
  int op[5];
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
