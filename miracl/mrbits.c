...
int logb2(_MIPD_ big x) { /* returns number of bits in x */
  int xl, lg2;
  mr_small top;
#ifdef MR_OS_THREADS
...
void sftbit(_MIPD_ big x, int n, big z) { /* shift x by n bits */
  int m;
  mr_small sm;
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
...
void expb2(_MIPD_ int n, big x) { /* sets x=2^n */
  int r, p;
#ifndef MR_ALWAYS_BINARY
  int i;
#endif
...
void bigbits(_MIPD_ int n, big x) { /* sets x as random < 2^n */
  mr_small r;
  mr_lentype wlen;
#ifdef MR_FP
  mr_small dres;
