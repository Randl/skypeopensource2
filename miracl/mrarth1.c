...
tcl1:
 ...
 out1:
    ...
    while (m < xl || carry > 0) { /* multiply each digit of x by n */
...
void premult(_MIPD_ big x, int n, big z) { /* premultiply a big number by an int z=x.n */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
tcl2:
 ...
 out2:
    ...
    for (i = xl - 1; i >= 0; i--) { /* divide each digit of x by n */
#ifdef MR_NOASM
...
int subdiv(_MIPD_ big x, int n, big z) {  /*  subdivide a big number by an int   z=x/n  *
...
int remain(_MIPD_ big x, int n) { /* return integer remainder when x divided by n */
  int r;
  mr_lentype sx;
#ifdef MR_FP
  mr_small dres;
  ...
  if (n == 2 && MR_REMAIN(mr_mip->base, 2) == 0) { /* fast odd/even check if base is even */
    MR_OUT
  ...
  if (n == 8 && MR_REMAIN(mr_mip->base, 8) == 0) { /* fast check */
    MR_OUT
...
void bytes_to_big(_MIPD_ int len, const char *ptr, big x) { /* convert len bytes into a big           *
...
int big_to_bytes(_MIPD_ int max, big x, char *ptr, BOOL justify) { /* convert big into octet string */
