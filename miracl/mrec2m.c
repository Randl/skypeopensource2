...
/* catch some nonsense conditions */
...
#ifndef MR_NO_SS
  mr_mip->SS = FALSE;
  if (m < 0) { /* its a supersingular curve! */
    mr_mip->SS = TRUE;
  ...
  if (!prepare_basis(_MIPP_ m, a, b, c, check)) { /* unable to set the basis */
    MR_OUT
    return FALSE;
  }
...
BOOL epoint2_set(_MIPD_ big x, big y, int cb, epoint *p) { /* initialise a point on active ecurve            *
...
#ifndef MR_NO_SS
  if (mr_mip->SS) { /* Super-singular - calculate x^3+x+B */
    copy(x, p->X);
...
BOOL epoint2_norm(_MIPD_ epoint *p) { /* normalise a point */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
#ifndef MR_AFFINE_ONLY
...
void epoint2_getxyz(_MIPD_ epoint *p, big x, big y, big z) {
  ...
  if (p->marker == MR_EPOINT_INFINITY) {
#ifndef MR_AFFINE_ONLY
    if (mr_mip->coord == MR_AFFINE) { /* (0,0) = O */
#endif
      if (x != NULL) zero(x);
      if (y != NULL) zero(y);
#ifndef MR_AFFINE_ONLY
    ...
    if (mr_mip->coord == MR_PROJECTIVE) { /* (1,1,0) = O */
...
int epoint2_get(_MIPD_ epoint *p, big x, big y) { /* Get point co-ordinates in affine, normal form       *
...
void ecurve2_double(_MIPD_ epoint *p) { /* double epoint on active curve */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif

  if (p->marker == MR_EPOINT_INFINITY) { /* 2 times infinity == infinity! */
    return;
  }
#ifndef MR_AFFINE_ONLY
  if (mr_mip->coord == MR_AFFINE) {
...
#endif
    if (size(p->X) == 0) { /* set to point at infinity */
...
static BOOL ecurve2_padd(_MIPD_ epoint *p, epoint *pa) { /* primitive add two epoints on the active ecurve pa+=p      *
  ...
  if (mr_mip->coord == MR_AFFINE) {
    ...
    if (size(mr_mip->w6) == 0) {  /* divide by zero */
  ...
  if (mr_mip->SS) { /* pa+=p */
    ...
    if (size(mr_mip->w1) == 0) {
      if (mr_compare(mr_mip->w2, mr_mip->w8) == 0) { /* point at infinity */
        epoint2_set(_MIPP_ NULL, NULL, 0, pa);
        return TRUE;
      }
...
*/
  ...
  if (p->marker != MR_EPOINT_NORMALIZED) {
    ...
    if (size(mr_mip->w2) == 0) {
      if (size(mr_mip->w1) == 0) { /* should have doubled! */
        return FALSE;
      }
      else {
        epoint2_set(_MIPP_ NULL, NULL, 0, pa);
...
big ecurve2_add(_MIPD_ epoint *p, epoint *pa) {  /* pa=pa+p; */
...
void epoint2_negate(_MIPD_ epoint *p) { /* negate a point */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
void ecurve2_multi_add(_MIPD_ int m, epoint **x, epoint **w) { /* adds m points together simultaneously, w[i]+=x[i] */
  int i, *flag;
  big *A, *B, *C;
  char *mem;
#ifdef MR_OS_THREADS
  ...
  if (mr_mip->coord == MR_AFFINE) {
    ...
    for (i = 0; i < m; i++) {
      ...
      flag[i] = 0;
      if (mr_compare(x[i]->X, w[i]->X) == 0 && mr_compare(x[i]->Y, w[i]->Y) == 0) { /* doubling */
        ...
        if (size(B[i]) == 0) { /* point at infinity */
          flag[i] = 1;                /* result is infinity */
          copy(mr_mip->w3, B[i]);
          continue;
        }
  ...
  else { /* no speed-up for projective coordinates */
    for (i = 0; i < m; i++) ecurve2_add(_MIPP_ x[i], w[i]);
  }
#endif
  MR_OUT
...
BOOL epoint2_multi_norm(_MIPD_
                        int m,
                        big *work,
                        epoint **p) { /* Normalise an array of points of length m<20 - requires a workspace array of length m */
...
static void table_init(_MIPD_ epoint *g, epoint **table) { /* A precomputation option for the multiplication of a  */
  ...
  if (mr_mip->KOBLITZ) {
    ...
    for (i = 1; i < MR_ECC_STORE_2M; i++) {  /* Consider i expressed as NAF */

      n = i;
      n3 = 3 * i + 1;
      t = n3;
      nb = 0;
      while (t > 1) { /* number of bits in n3 */
        t >>= 1;
        nb++;
      }
      while (nb > nf) { /* move to next power of tau */
        frobenius(_MIPP_ table[MR_ECC_STORE_2M - 1]);
        nf++;
      }
...
static void prepare_naf(_MIPD_ big e, big hp, big hn) { /* prepare NAF - exponent = hp-hn = (3e-e)/2 */
...
void ecurve2_mult(_MIPD_ big e, epoint *pa, epoint *pt) { /* pt=e*pa; */
...
/*    int coord;  */
 ...
 /*   char mem[MR_ECP_RESERVE_A(MR_ECC_STORE_2M)];   Reserve space for AFFINE (x,y) only */
  ...
  if (size(mr_mip->w9) == 0 && size(mr_mip->w10) == 0) { /* multiplied by 0 */
    epoint2_set(_MIPP_ NULL, NULL, 0, pt);
    MR_OUT
    return;
  }
...
/*        mr_mip->coord=coord; */
    ...
    while (size(mr_mip->w11) > 0) { /* add/subtract method */
      if (mr_mip->user != NULL) (*mr_mip->user)();
...
void ecurve2_multn(_MIPD_ int n, big *y, epoint **x, epoint *w) { /* pt=e[o]*p[0]+e[1]*p[1]+ .... e[n-1]*p[n-1]   */
  int i, j, k, m, nb, ea;
  epoint **G;
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
...
void ecurve2_mult2(_MIPD_ big e, epoint *p, big ea, epoint *pa, epoint *pt) { /* pt=e*p+ea*pa; */
  ...
  while (size(mr_mip->w11) > 0) { /* add/subtract method */
    ...
    e1 = h1 = e2 = h2 = 0;
    if (mr_compare(mr_mip->w9, mr_mip->w11) >= 0) { /* e1(i)=1? */
      e2 = 1;
      mr_psub(_MIPP_ mr_mip->w9, mr_mip->w11, mr_mip->w9);
    }
    if (mr_compare(mr_mip->w10, mr_mip->w11) >= 0) { /* h1(i)=1? */
      h2 = 1;
      mr_psub(_MIPP_ mr_mip->w10, mr_mip->w11, mr_mip->w10);
    }
    if (mr_compare(mr_mip->w12, mr_mip->w11) >= 0) { /* e2(i)=1? */
      e1 = 1;
      mr_psub(_MIPP_ mr_mip->w12, mr_mip->w11, mr_mip->w12);
    }
    if (mr_compare(mr_mip->w13, mr_mip->w11) >= 0) { /* h2(i)=1? */
      h1 = 1;
...
BOOL ebrick2_init(_MIPD_ ebrick2 *B, big x, big y, big a2, big a6, int m, int a, int b, int c, int window, int nb) { /* (x,y) is the fixed base                            *
