...
static int expon(_MIPD_ big w, int n) {  /* generator for C.F. of e */
  if (n == 0) return 2;
  if (n % 3 == 2) return 2 * (n / 3) + 2;
  else return 1;
}

void fexp(_MIPD_ flash x, flash y) { /* calculates y=exp(x) */
  ...
  if (n == 0) convert(_MIPP_ 1, y);
    ...
    if (minus) { /* underflow to zero - bit of a bodge */
      rem = mr_mip->ERCON;
      mr_mip->ERCON = TRUE;
      fpower(_MIPP_ y, n, y);
      mr_mip->ERCON = rem;
  ...
  op[1] = 1;
  op[2] = 0;
  for (m = sqrn; m > 0; m--) { /* Unwind C.F. expansion for exp(x)-1 */
...
void flog(_MIPD_ flash x, flash y) { /* calculate y=log(x) to base e */
  BOOL hack;
  int op[5];
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
  ...
  while (mr_mip->workprec != mr_mip->nib) { /* Newtons iteration w11=w11+(y-exp(w11))/exp(w11) */
...
void fpowf(_MIPD_ flash x, flash y, flash z) { /* raise flash number to flash power *
   *     z=x^y  -> z=exp(y.log(x))     */
  int n;
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
  ...
  n = size(y);
  if (mr_abs(n) < MR_TOOBIG) { /* y is small int */
    fpower(_MIPP_ x, n, z);
    MR_OUT
    return;
  }
  ...
  n = size(mr_mip->w13);
  if (mr_abs(n) < MR_TOOBIG) { /* 1/y is small int */
    froot(_MIPP_ x, n, z);
    MR_OUT
    return;
  }
