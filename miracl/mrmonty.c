...
/* Is it set-up already? */
  if (size(mr_mip->modulus) != 0) if (mr_compare(n, mr_mip->modulus) == 0) return mr_mip->ndash;
...
/* Did you know that for p=2 mod 3, -3 is a QNR? */
  ...
  if (!mr_mip->MONTY) { /* Montgomery arithmetic is turned off */
  ...
  if (invmodp(_MIPP_ mr_mip->w15, mr_mip->w6, mr_mip->w14) != 1) { /* problems */
    mr_berror(_MIPP_ MR_ERR_BAD_MODULUS);
    MR_OUT
    return (mr_small) 0;
  }
...
void nres(_MIPD_ big x, big y) { /* convert x to n-residue format */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
void redc(_MIPD_ big x, big y) { /* Montgomery's REDC function p. 520 */
...
void nres_complex(_MIPD_ big a, big b, big r, big i) {
  ...
  if (mr_mip->NO_CARRY && mr_mip->qnr
      == -1) { /* if modulus is small enough we can ignore carries, and use simple addition and subtraction */
...
void nres_modadd(_MIPD_ big x, big y, big w) { /* modular addition */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
#ifdef MR_COUNT_OPS
...
void nres_modsub(_MIPD_ big x, big y, big w) { /* modular subtraction */
...
int nres_moddiv(_MIPD_ big x, big y, big w) { /* Modular division using n-residues w=x/y mod n */
  int gcd;
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
void nres_premult(_MIPD_ big x, int k, big w) { /* multiply n-residue by small ordinary integer */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
  int sign = 0;
...
void nres_modmult(_MIPD_ big x, big y, big w) { /* Modular multiplication using n-residues w=x*y mod n */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
BOOL nres_double_inverse(_MIPD_ big x, big y, big w, big z) { /* find y=1/x mod n and z=1/w mod n */
...
BOOL nres_multi_inverse(_MIPD_ int m, big *x, big *w) { /* find w[i]=1/x[i] mod n, for i=0 to m-1 *
...
void ecurve_init(_MIPD_ big a, big b, big p, int type) { /* Initialize the active ecurve    *
