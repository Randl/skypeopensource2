...
BOOL crt_init(_MIPD_ big_chinese *c, int r, big *moduli) { /* calculate CRT constants */
  int i, j, k;
#ifdef MR_OS_THREADS
...
void crt_end(big_chinese *c) { /* clean up after CRT */
  int i, j, k;
  if (c->NP < 2) return;
...
void crt(_MIPD_ big_chinese *c, big *u, big x) { /* Chinese Remainder Thereom                  *
  ...
  copy(u[0], c->V[0]);
  for (k = 0, i = 1; i < c->NP; i++) { /* Knuth page 274 */
