...
void gprime(_MIPD_ int maxp) { /* generate all primes less than maxp into global PRIMES */
  char *sv;
  int pix, i, k, prime;
#ifdef MR_OS_THREADS
  ...
  for (i = 0; i < maxp; i++)
    sv[i] = TRUE;
  for (i = 0; i < maxp; i++)
    if (sv[i]) { /* using sieve of Eratosthenes */
      prime = i + i + 3;
...
#endif
  for (i = 0; mr_mip->PRIMES[i] != 0; i++) { /* test for divisible by first few primes */
    while (subdiv(_MIPP_ y, mr_mip->PRIMES[i], mr_mip->w1) == 0) {
      if (x == y) {
        ...
        if (size(mr_mip->w1) == 1) { /* y is largest prime factor */
          MR_OUT
          return 1;
        }
        copy(mr_mip->w1, y);
...
BOOL isprime(_MIPD_ big x) {  /*  test for primality (probably); TRUE if x is prime. test done NTRY *
...
/* Miller-Rabin */
  ...
  for (n = 1; n <= times; n++) { /* perform test times times */
    j = 0;
#ifndef MR_NO_RAND
    do {
...
/* use small random numbers...  */
    ...
    while ((j > 0 || size(mr_mip->w9) != 1)
      ...
      j++;
      if ((j > 1 && size(mr_mip->w9) == 1) || j == k) { /* definitely not prime */
        MR_OUT
        return FALSE;
      }
...
BOOL nxprime(_MIPD_ big w, big x) {  /*  find next highest prime from w using     *
...
BOOL nxsafeprime(_MIPD_ int type, int subset, big w, big p) { /* If type=0 finds next highest "safe" prime p >= w *
