...
int encode41_sess1pkt_error2_recurs(char *buf, int buf_limit_len) {
  struct blob_s blob;
  uint session_id;
  uint session_cmd;
  int buf_len;
  ...
  blob.obj_data = get_remote_cmd0D_streamid();
  blob.data_ptr = 0;
  blob.data_size = 0;
  buf_len = make_41encode(buf, buf_len, (char *) &blob, 0);
...
int encode41_sess1pkt_error2(char *buf, int buf_limit_len) {
  struct blob_s blob;
  uint session_id;
  uint session_cmd;
  int buf_len;
  ...
  intbuf_len = encode41_sess1pkt_error2_recurs(intbuf, sizeof(intbuf));
  blob.obj_type = 5;
  blob.obj_index = 3;
  blob.obj_data = 0;
