...
// sess1pkt53
//
int encode41_sess1pkt53_recurs(char *buf, int buf_limit_len) {
  struct blob_s blob;
  int buf_len;
  int blobs_count;
  ...
  blob.obj_index = 0x03;
  blob.obj_data = 0x00;
  blob.data_ptr = REMOTE_AUTHORIZED188;
...
int encode41_sess1pkt_53(char *buf, int buf_limit_len, char *chatstr) {
  struct blob_s blob;
  uint session_id;
  uint session_cmd;
  int buf_len;
  ...
  intbuf_len = encode41_sess1pkt53_recurs(intbuf, sizeof(intbuf));
  blob.obj_type = 5;
  blob.obj_index = 0x03;
  blob.obj_data = 0;
