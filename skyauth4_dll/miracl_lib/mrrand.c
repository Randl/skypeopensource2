...
void bigrand(_MIPD_ big w, big x) {  /*  generate a big random number 0<=x<w  */
  int m;
  mr_small r;
#ifdef MR_FP
  mr_small dres;
  ...
  do { /* create big rand piece by piece */
    m++;
    mr_mip->w0->len = m;
    r = brand(_MIPPO_);
...
void bigdig(_MIPD_ int n, int b, big x) { /* generate random number n digits long *
   * to "printable" base b                */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
