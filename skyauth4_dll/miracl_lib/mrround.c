...
static int euclid(_MIPD_ big x, int num) { /* outputs next c.f. quotient from gcd(w5,w6) */
  mr_small sr, m;
#ifdef MR_FP
  mr_small dres;
#endif
  ...
  if (num == 0) {
    ...
    mr_mip->last = FALSE;
    if (mr_compare(mr_mip->w6, mr_mip->w5) > 0) { /* ensure w5>w6 */
  ...
  mr_mip->carryon = FALSE;
  if (mr_mip->b == 0) { /* update w5 and w6 */
    mr_mip->check = OFF;
...
void mround(_MIPD_ big num, big den, flash z) { /* reduces and rounds the fraction num/den into z */
  int s;
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
