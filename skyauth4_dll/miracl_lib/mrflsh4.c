...
void ftanh(_MIPD_ flash x, flash y) { /* calculates y=tanh(x) */
  int op[5];
#ifdef MR_OS_THREADS
...
void fatanh(_MIPD_ flash x, flash y) { /* calculate y=atanh(x) */
  int op[5];
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
void fsinh(_MIPD_ flash x, flash y) { /*  calculate y=sinh(x) */
  int op[5];
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
void fasinh(_MIPD_ flash x, flash y) { /* calculate y=asinh(x) */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
  copy(x, y);
...
void fcosh(_MIPD_ flash x, flash y) { /* calculate y=cosh(x) */
  int op[5];
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
void facosh(_MIPD_ flash x, flash y) { /* calculate y=acosh(x) */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
  copy(x, y);
