...
static mr_small qdiv(mr_large u, mr_large v) { /* fast division - small quotient expected.  */
  mr_large lq, x = u;
#ifdef MR_FP
  mr_small dres;
#endif
...
int xgcd(_MIPD_ big x, big y, big xd, big yd, big z) { /* greatest common divisor by Euclids method  *
  ...
  while (size(mr_mip->w2) != 0) {
    ...
    if (n == 1) {
      ...
      if (mr_mip->base == 0) {
        ...
        if (n > 2 && m != 0) { /* squeeze out as much significance as possible */
    ...
    forever { /* work only with most significant piece */
      if (last) {
        if (v == 0) break;
        q = qdiv(u, v);
        if (q == 0) break;
...
BOOL multi_inverse(_MIPD_ int m, big *x, big n, big *w) { /* find w[i]=1/x[i] mod n, for i=0 to m-1 *
