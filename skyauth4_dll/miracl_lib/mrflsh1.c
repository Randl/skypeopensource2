...
static int quad(_MIPD_ big w, int n) { /* generator for C.F. of square root of small integer */
  int t;
#ifdef MR_OS_THREADS
...
void fpower(_MIPD_ flash x, int n, flash w) { /* raise floating-slash number to integer power w=x^n */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
  copy(x, mr_mip->w8);
...
BOOL froot(_MIPD_ flash x, int n, flash w) { /* extract nth root of x  - w=x^(1/n) using Newtons method */
  ...
  hack = FALSE;
  if (mr_lent(w) <= 2) { /* for 'simple' w only */
    hack = TRUE;
  ...
  while (mr_mip->workprec != mr_mip->nib) { /* Newtons iteration w10=(w/w10^(n-1)+(n-1)*w10)/n */
