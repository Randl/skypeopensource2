...
mr_small normalise(_MIPD_ big x, big y) { /* normalise divisor */
  mr_small norm, r;
#ifdef MR_FP
  mr_small dres;
#endif
...
void multiply(_MIPD_ big x, big y, big z) {  /*  multiply two big numbers: z=x.y  */
  int i, xl, yl, j, ti;
...
ASM (
    ...
    if (x == y && xl > SQR_FASTER_THRESHOLD) { /* squaring can be done nearly twice as fast */
      for (i = 0; i < xl - 1; i++) { /* long multiplication */
        carry = 0;
        for (j = i + 1; j < xl; j++) { /* Only do above the diagonal */
#ifdef MR_NOASM
...
void divide(_MIPD_ big x, big y, big z) {  /*  divide two big numbers  z=x/y : x=x mod y  *
  ...
  if (x0 == y0) {
    if (x0 == 1) /* special case - x and y are both mr_smalls */
    {
      ...
      while (mr_compare(w0, y) >= 0) {  /* mr_small quotient - so do up to four subtracts instead */
  ...
  if (y0 == 1) {  /* y is int - so use subdiv instead */
#ifdef MR_FP_ROUNDING
...
BOOL divisible(_MIPD_ big x, big y) { /* returns y|x, that is TRUE if y divides x exactly */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
void mad(_MIPD_ big x, big y, big z, big w, big q, big r) { /* Multiply, Add and Divide; q=(x*y+z)/w remainder r   *
