...
int instr(_MIPD_ flash x, char *string) {  /*  input a big number       *
  ...
  if (mr_mip->INPLEN == 0) { /* inputting ASCII bytes */

#ifndef MR_NO_FILE_IO

    if (mr_mip->fin) { /* read in characters */
      i = 0;
      do {
  ...
  frac = FALSE;
#endif
  if (mr_mip->INPLEN > 0 && mr_mip->apbase <= 60) { /* skip leading blanks and check sign */
#ifdef MR_FLASH
    if (string[mr_mip->INPLEN - 1] == '/') mr_mip->INPLEN--;
#endif
    while (string[e] == ' ') e++;
    if (string[e] == '-') { /* check sign */
      s = MINUS;
      e++;
    }
    if (string[e] == '+') e++;
  ...
  for (i = mr_mip->INPLEN - 1; i >= e; i--) {
    ...
    if (mr_mip->apbase <= 60 || mr_mip->apbase == 64) { /* check for slash or dot and convert character to number */
#ifdef MR_FLASH
...
int otstr(_MIPD_ flash x, char *string) {  /*  output a big number  */
  ...
  if (mr_mip->RPOINT) { /* output with radix point */
    denom(_MIPP_ x, mr_mip->w5);
    if (size(mr_mip->w5) > 1) { /* multiply up numerator to get full precision in *
  ...
  forever {
    ...
    for (i = m; i > 0; i--) {
      ...
      if (i > nd) ch = '0';
        ...
        if (mr_mip->apbase <= 60) { /* convert number to character */
          ch += 48;
          if (ch >= 58) ch += 7;
          if (ch >= 91) ch += 6;
        }
  ...
  if (mr_mip->apbase == 64) { /* base64 padding */
    while (n % 3 != 0) {
#ifndef MR_NO_FILE_IO
      if (!mr_mip->fout) string[n] = '=';
      else fputc('=', mr_mip->otfile);
...
int innum(_MIPD_ flash x, FILE *filep) { /* convert from string to flash x */
  int n;
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
int otnum(_MIPD_ flash x, FILE *filep) { /* convert flash to string */
  int n;
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
