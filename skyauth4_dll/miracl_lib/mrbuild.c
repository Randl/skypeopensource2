...
void build(_MIPD_ flash x, int (*gen)(_MIPT_ big, int)) { /* Build x from its regular c.f. *
   * generated by gen()            */
  ...
  while (!mr_mip->ERNUM && q >= 0) {
    if (q == MR_TOOBIG || n == 0 || finoff) {
      ...
      mr_mip->check = ON;
      if ((int) (mr_mip->w7->len & MR_OBITS) > mr_mip->nib) { /* oops! */
    ...
    if (lz >= prc - 1) { /* nearly finished - so be careful not to overshoot */
      if (mr_mip->base == 0) {
    ...
    forever {
      q = (*gen)(_MIPP_ x, n);
      if (q < 0 || q >= MR_TOOBIG / mr_abs(d)) { /* there could be more.... *** V3.21 mod *** */
        last = FALSE;
        break;
      }
      rm = b - q * d;
