...
static mr_unsign32 sbrand(csprng *rng) { /* Marsaglia & Zaman random number generator */
  int i, k;
  mr_unsign32 pdiff, t;
  rng->rndptr++;
...
static void sirand(csprng *rng, mr_unsign32 seed) { /* initialise random number system */
...
static void fill_pool(csprng *rng) { /* hash down output of RNG to re-fill the pool */
  int i;
  sha sh;
  shs_init(&sh);
...
void strong_init(csprng *rng, int rawlen, char *raw, mr_unsign32 tod) { /* initialise from at least 128 byte string of raw  *
...
void strong_kill(csprng *rng) { /* kill internal state */
  int i;
...
void strong_bigdig(_MIPD_ csprng *rng, int n, int b, big x) { /* generate random number n digits long *
   * to "printable" base b                */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
