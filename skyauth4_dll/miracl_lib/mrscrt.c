...
static mr_utype in_range(mr_utype x, mr_utype y) { /* x=x%y, and positive */
  mr_utype r;
#ifdef MR_FP
  mr_small dres;
#endif
...
BOOL scrt_init(_MIPD_
               small_chinese *c,
               int r,
               mr_utype *moduli) { /* calculate CRT constants - returns FALSE if there is a problem */
  int i, j, k;
  if (r < 1) return FALSE;
  if (r == 1) {
    c->NP = 1;
...
void scrt_end(small_chinese *c) { /* clean up after CRT */
  if (c->NP < 1) {
    c->NP = 0;
    return;
  }
...
void scrt(_MIPD_ small_chinese *c, mr_utype *u, big x) { /* Chinese Remainder Thereom                  *
  ...
  V[0] = u[0];
  for (k = 0, i = 1; i < c->NP; i++) { /* Knuth P. 274 */
    V[i] = u[i] - V[0];
#ifdef MR_FP_ROUNDING
    im=mr_invert(M[i]);
