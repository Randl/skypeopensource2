...
void flop(_MIPD_ flash x, flash y, int *op, flash z) { /* Do basic flash operation - depending on   *
  ...
  for (i = 1; i <= 4; i++) {
    ...
    switch (code) {
      case 0 :
        if (x == y) multiply(_MIPP_ mr_mip->w1, mr_mip->w1, mr_mip->w0);
        else multiply(_MIPP_ mr_mip->w1, mr_mip->w3, mr_mip->w0);
        break;
      case 1 :
        multiply(_MIPP_ mr_mip->w1, mr_mip->w4, mr_mip->w0);
        break;
      case 2 :
        multiply(_MIPP_ mr_mip->w2, mr_mip->w3, mr_mip->w0);
        break;
      case 3 :
        if (x == y) multiply(_MIPP_ mr_mip->w2, mr_mip->w2, mr_mip->w0);
    ...
    switch (i) {
      case 1 :
        copy(mr_mip->w0, mr_mip->w5);
        break;
      case 2 :
        add(_MIPP_ mr_mip->w5, mr_mip->w0, mr_mip->w5);
        break;
      case 3 :
        copy(mr_mip->w0, mr_mip->w6);
        break;
      case 4 :
        add(_MIPP_ mr_mip->w6, mr_mip->w0, mr_mip->w6);
        break;
    }
  }
  mr_mip->check = ON;
...
void fmul(_MIPD_ flash x, flash y, flash z) { /* Flash multiplication - z=x*y */
  int op[5];
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
void fdiv(_MIPD_ flash x, flash y, flash z) { /* Flash divide - z=x/y */
  int op[5];
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
void fadd(_MIPD_ flash x, flash y, flash z) { /* Flash add - z=x+y */
  int op[5];
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
void fsub(_MIPD_ flash x, flash y, flash z) { /* Flash subtract - z=x-y */
  int op[5];
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
void ftrunc(_MIPD_ flash x, big y, flash z) { /* sets y=int(x), z=rem(x) - returns *
...
void fmodulo(_MIPD_ flash x, flash y, flash z) { /* sets z=x mod y */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
  if (mr_mip->ERNUM) return;
...
void fconv(_MIPD_ int n, int d, flash x) { /* convert simple fraction n/d to Flash form */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
  if (mr_mip->ERNUM) return;
...
void frecip(_MIPD_ flash x, flash y) { /* set y=1/x */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
  if (mr_mip->ERNUM) return;
...
void fpmul(_MIPD_ flash x, int n, int d, flash y) { /* multiply x by small fraction n/d - y=x*n/d */
  int r, g;
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
void fincr(_MIPD_ flash x, int n, int d, flash y) { /* increment x by small fraction n/d - y=x+(n/d) */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
  if (mr_mip->ERNUM) return;
