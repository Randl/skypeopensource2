...
void nres_powltr(_MIPD_ int x, big y, big w) { /* calculates w=x^y mod z using Left to Right Method   */
  ...
  if (mr_mip->base == mr_mip->base2) {
    ...
    nres(_MIPP_ w, w);
    if (nb > 1)
      for (i = nb - 2; i >= 0; i--) { /* Left to Right binary method */

        if (mr_mip->user != NULL) (*mr_mip->user)();

        nres_modmult(_MIPP_ w, w, w);
        if (mr_testbit(_MIPP_ mr_mip->w1, i)) { /* this is quick */
    ...
    while (size(mr_mip->w2) != 0) { /* Left to Right binary method */
...
void powmodn(_MIPD_ int n, big *x, big *y, big p, big w) {/* w=x[0]^y[0].x[1]^y[1] .... x[n-1]^y[n-1] mod n */
  int j;
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
...
void nres_powmod2(_MIPD_ big x, big y, big a, big b, big w) { /* finds w = x^y.a^b mod n. Fast for some cryptosystems */
  ...
  if (mr_mip->base == mr_mip->base2) { /* uses 2-bit sliding window. This is 25% faster! */
#endif
...
void powmod2(_MIPD_ big x, big y, big a, big b, big n, big w) {/* w=x^y.a^b mod n */
#ifdef MR_OS_THREADS
  miracl *mr_mip=get_mip();
#endif
  if (mr_mip->ERNUM) return;
...
void powmod(_MIPD_ big x, big y, big n, big w) { /* fast powmod, using Montgomery's method internally */
  ...
  else if (subdivisible(_MIPP_ n, 2)) mty = FALSE;

  if (!mty) { /* can't use Montgomery */
    copy(y, mr_mip->w1);
    copy(x, mr_mip->w3);
    zero(w);
    if (size(mr_mip->w3) == 0) {
  ...
  else { /* optimized code for odd moduli */
    prepare_monty(_MIPP_ n);
...
/* Is Montgomery modulus in use... */
  ...
  clean_up = FALSE;
  if (mty) { /* still a chance to use monty... */
    if (size(mr_mip->modulus) != 0) { /* somebody else is using it */
      if (mr_compare(n, mr_mip->modulus) != 0) mty = FALSE;
    }
    else { /* its unused, so use it, but clean up after */
      clean_up = TRUE;
    }
  }
  if (!mty) { /* can't use Montgomery! */
...
void nres_powmod(_MIPD_ big x, big y, big w) {  /*  calculates w=x^y mod z, using m-residues       *
...
#ifndef MR_ALWAYS_BINARY
  if (mr_mip->base == mr_mip->base2) { /* build a table. Up to 5-bit sliding windows. Windows with
    ...
    copy(mr_mip->w3, w);
    if (nb > 1)
      for (i = nb - 2; i >= 0;) { /* Left to Right method */
    ...
    forever { /* "Russian peasant" Right-to-Left exponentiation */
