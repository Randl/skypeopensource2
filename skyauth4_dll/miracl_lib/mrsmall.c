...
mr_small smul(mr_small x, mr_small y, mr_small n) { /* returns x*y mod n */
  mr_small r;
...
mr_small invers(mr_small x, mr_small y) { /* returns inverse of x mod y */
  mr_small r, s, q, t, p;
#ifdef MR_FP
  mr_small dres;
#endif
  ...
  pos = TRUE;
#ifndef MR_NOFULLWIDTH
  if (p == 0) { /* if modulus is 0, assume its actually 2^MIRACL */
...
int jac(mr_small x, mr_small n) { /* finds (x/n) as (-1)^m */
  int m, k, n8, u4;
  mr_small t;
#ifdef MR_FP
  mr_small dres;
...
mr_small spmd(mr_small x, mr_small n, mr_small m) { /*  returns x^n mod m  */
  mr_small r, sx;
#ifdef MR_FP
  mr_small dres;
#endif
...
mr_small sqrmp(mr_small x, mr_small m) { /* square root mod a small prime by Shanks method  *
  ...
  if (MR_REMAIN(m, 8) == 5) { /* also relatively easy */
