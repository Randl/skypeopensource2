...
static MR_BYTE bmul(MR_BYTE x, MR_BYTE y) { /* x.y= AntiLog(Log(x) + Log(y)) */
...
static MR_BYTE product(MR_WORD x, MR_WORD y) { /* dot product of two 4-byte arrays */
  MR_BYTE xb[4], yb[4];
  unpack(x, xb);
  unpack(y, yb);
...
static MR_WORD InvMixCol(MR_WORD x) { /* matrix Multiplication */
  MR_WORD y, m;
  MR_BYTE b[4];
...
void aes_reset(aes *a, int mode, char *iv) { /* reset mode, or reset iv */
  int i;
  a->mode = mode;
  for (i = 0; i < 4 * NB; i++)
    a->f[i] = 0;
...
BOOL aes_init(aes *a, int mode, int nk, char *key, char *iv) { /* Key=nk bytes */
