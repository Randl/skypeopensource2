...
static void shs_transform(sha *sh) { /* basic transformation step */
  ...
  d = sh->h[3];
  e = sh->h[4];
  for (t = 0; t < 20; t++) { /* 20 times - mush it up */
  ...
  for (t = 20; t < 40; t++) { /* 20 more times - mush it up */
  ...
  for (t = 40; t < 60; t++) { /* 20 more times - mush it up */
  ...
  for (t = 60; t < 80; t++) { /* 20 more times - mush it up */
...
void shs_init(sha *sh) { /* re-initialise */
  int i;
...
void shs_process(sha *sh, int byte) { /* process the next message byte */
  int cnt;
...
void shs_hash(sha *sh, char hash[20]) { /* pad message and finish - supply digest */
  int i;
  ...
  sh->w[15] = len0;
  shs_transform(sh);
  for (i = 0; i < 20; i++) { /* convert to bytes */
