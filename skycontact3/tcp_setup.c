...
int make_dh384_handshake(char *ip, unsigned short port){
	...
	// forming pkt1
	send=malloc(0x1000);
	Skype_Handshake(send, &send_len);	
	printf("Handshake v5.5 1 pkt len: 0x%08X\n",send_len);


	// Display pkt before sending
	show_memory(send, send_len, "Send pkt");
	// Sending packet
	len=tcp_talk(ip,port,send,send_len,result,0);
	if (len<=0) {
		printf("recv timeout\n");
		return -1;
	};
	if (len>=1023) {
		...
		printf("Too big pkt recv, return...\n");
		return -1;
	};
	// Display received pkt
	show_memory(result, len, "Recv");


	// prepare and forming pkt2
