...
int tcp_talk(char *remoteip, unsigned short remoteport, char *buf, int len, char *result, int need_close) {
  ...
  //debuglog("tcp_talk before send\n");
  ret = send(sock, buf, len, 0);
  if (ret < 0) {
    debuglog("Send error\n");
    return -1;
  };
  //debuglog("tcp_talk after send\n");

  memset(tmpbuf, 0, sizeof(tmpbuf));
  //debuglog("tcp_talk before recv_1\n");
  cnt = recv(sock, tmpbuf, 0x10000, 0);
  //debuglog("tcp_talk after recv_1\n");
  //debuglog("tcp_talk recv_1 len: 0x%08X\n", cnt);
  if (cnt < 0) {
    debuglog("Recv_1 error\n");
    return -1;
  };
  memcpy(result, tmpbuf, cnt);
  if (cnt <= 5) {
    memset(tmpbuf, 0, sizeof(tmpbuf));
    //debuglog("tcp_talk before recv_2\n");
    cnt2 = recv(sock, tmpbuf, 0x10000, 0);
    //debuglog("tcp_talk after recv_2\n");
    if (cnt2 < 0) {
      debuglog("Recv2 error\n");
      return -1;
    };
...
int tcp_talk_recv(char *remoteip, unsigned short remoteport, char *result, int need_close) {
  ...
  memset(tmpbuf, 0, sizeof(tmpbuf));
  //debuglog("tcp_talk_recv before recv\n");
  cnt = recv(sock, tmpbuf, 0x10000, 0);
  //debuglog("tcp_talk_recv after recv\n");
  if (cnt < 0) {
    debuglog("[tcp_talk_recv] Recv error\n");
    return -1;
  };
  memcpy(result, tmpbuf, cnt);
