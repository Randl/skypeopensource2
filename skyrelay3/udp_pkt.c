...
*/
  ...
  u8 INIT_UNK[] =
      "\x9C\xCB\x4F\xAB\x1F\xA6\xA3\x68\x01\xC0\xA8\x01\x87\x38\x4A\x9D"
          "\x37\x38\x95\x9C\x57\xB2\x44\x96\x26\x38\x4A";
  u32 INIT_UNK_len = sizeof(INIT_UNK) - 1;
...
// udp request Search //
////////////////////////
int make_udprelay_pkt3(char *ourip,
                       char *destip,
                       u16 seqnum,
                       u32 rnd,
                       char *remote_name,
...
int process_udprelay_pkt1(char *pkt, int pkt_len, char *ourip, char *destip) {
  ...
  if (pkt_len < 0x100) {
    printf("returning size too small\n");
  };
  if (pkt_len == 0x0B) {
    printf("--- rc4 decoding fail on remote side, check you public ip addr ---\n");
  };
  if (pkt_len == 0x11) {
    printf("Get error code (42 unpack error?)\n");
  };
  if (pkt_len == 0x12) {
    printf("Success!\n");
  };
  ...
  if (memcmp(pkt + header_len, "\x04\x03", 2) == 0) {
    printf("Got 04 03 error code (some unknown)\n");
  };

  if (memcmp(pkt + header_len, "\x04\xB3\x04", 3) == 0) {
    printf("Got B3 04 code. Success!\n");
  };
...
int process_udprelay_pkt3(char *pkt, int pkt_len, char *ourip, char *destip) {
  ...
  if (pkt_len == 0x0B) {
    printf("--- Error. RC4 decoding fail on remote side, check you public ip addr ---\n");
  };
  if (pkt_len == 0x11) {
    printf("Get error code (42 unpack error?)\n");
  };
  ...
  */
  if (memcmp(pkt + header_len, "\x07\xB3\x03", 3) == 0) {
    printf("Got B3 03 code. User offline (or connect on vcard ip failed?).\n");
    return -1;
  };
  if (memcmp(pkt + header_len, "\x04\xD3\x05", 3) == 0) {
    printf("Got D3 05 code. Some error. User vcard mismatch.\n");
    return -1;
  };

/*
07 DB 05 5E 27 41 01 00 12 05 
{
00-12: 05 00 00 00
}
