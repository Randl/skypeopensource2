// relaysend2.c: Defines the entry point for the console application.
//
...
int relaysend_main(char* static_myip, char* static_username, char* static_uservcard, char* static_msg){
    ...
    */
    ...
    UnicodeToUtf8(MSG_TEXT, static_msg, 0x1000);

	debuglog_info("UserName: %s\n", REMOTE_NAME);
	debuglog_info("UserVCard: %s\n", uservcard);
	debuglog_info("MyAddr: %s\n", MY_ADDR);
	debuglog_info("MSG: %s\n", MSG_TEXT);
    // end of converting
...
int on_relay_success() {
    ...
    ret = relaysend_skypeclient_tcpconnect_sess1();
...
int relaysend_skypeclient_tcpconnect_sess1(){
    char *ip;
    unsigned short port;
