...
int status_udp_print(int retcode, char *our_public_ip) {
  ...
  switch (retcode) {
    case 1:
      debuglog("this is supernode! our public ip: %s\n", our_public_ip);
      break;
    case -1:
...
int snode_udp_push1(char *destip, u16 destport, char *our_public_ip) {
  ...
  //if (DEBUG) debuglog("part len:0x%08X\n",resp_len);
  debuglog("part len:0x%08X\n", resp_len);
...
int main_skypush_getrelays(char *myip, char *destip, unsigned short destport) {
  ...
  debuglog("Push request to target node ip: %s\n", destip);
  debuglog("Our IP: %s\n", our_public_ip);
...
//
int main_skypush_connect(char *myip,
                         char *destip,
  ...
  if (0) {
    global_connectip = strdup("62.165.23.175");
    global_connectport = 14410;
  };

  debuglog("Push request to target node ip: %s\n", destip);
  debuglog("Our IP: %s\n", our_public_ip);
  debuglog("Got conn_id: 0x%08X\n", global_connid);
