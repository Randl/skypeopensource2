...
int process_aes_crypt(char *data, int datalen, int usekey, int blkseq, int need_xor) {
  ...
  if (debug) show_memory(data, datalen, "Before AES crypt");
  ...
  if (debug) show_memory(data, datalen, "After AES crypt");
...
int process_aes(char *buf, int buf_len, int usekey, int blkseq, int need_xor) {
  ...
  if (debug) debuglog("aes_checksum_crc32=0x%08X\n", aes_checksum_crc32);
  ...
  if (debug) debuglog("crc32(after aes crypt)=%08X\n", pkt_crc32);
