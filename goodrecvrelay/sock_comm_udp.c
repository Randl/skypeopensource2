...
int udp_talk(char *remoteip,
  ...
  if (bind_local) {
    ...
    if (ret == -1) {
      debuglog("Bind localport failed\n");
      debuglog("WSAStartup error. Error: %d\n", WSAGetLastError());
      return -1;
    };
  };
  ...
  if (FD_ISSET(s, &rfds)) {
    ...
    if (ret < 0) {
      //debuglog("Recvfrom error\n");
      //debuglog("Error: recvfrom, Error code %d\n",WSAGetLastError());
  ...
  */
  ...
  do {
    ...
    if (FD_ISSET(s, &rfds)) {
      ...
      //debuglog("ret2: 0x%08X, ret: 0x%08X\n",ret2,ret);
      if (ret2 < 0) {
        //debuglog("Recvfrom error\n");
        //debuglog("Error: recvfrom, Error code %d\n",WSAGetLastError());
        //10054 - Connection reset by peer
