...
u8 REMOTE_CHAT_STRING[0x100];

uint GOT_REMOTE_MSG_COUNT;

uint HEADER_ID_CMD13R3_ID;
uint HEADER_ID_CMD13R4_ID;

char REMOTE_MSG[0x1000];
uint REMOTE_MSG_LEN;

char *RECV_MANYMSG_BUF;
uint RECV_MANYMSG_BUF_LEN;

uint RECV_MANYMSG_00_06_size_total;
uint RECV_MANYMSG_00_06_size_recv;
uint RECV_MANYMSG_00_06_flag;

uint global_unknown_cmd24_time;
uint global_unknown_cmd24_time_sec;
uint global_unknown_cmd24_blob1b;
...
unsigned int make_setup_global_init() {
  ...
  memcpy(LOCAL_UIC,
         "\x00\x00\x01\x04\x00\x00\x00\x01\x77\x9E\x0F\xA9\x19\xE7\xFD\x5A"
             ...
             "\x9F\x80\xA3\x50\x58\xD5\xC1\xFE", 0x189);
  ...
  memcpy(REMOTE_AUTHORIZED188,
         "\x00\x00\x01\x04\x00\x00\x00\x01\x94\x8E\xD8\xA1\x58\x6D\x7A\x36"
             ...
             "\xA0\xFD\x89\xF1\x1B\x36\xFF\x9A", 0x189);
  ...
  memcpy(AFTER_CRED2,
         "\x4B\xBB\xBB\xBB\xBB\xBB\xBB\xBB\xBB\xBB\xBB\xBB\xBB\xBB\xBB\xBB"
             ...
             "\x8B\xA2\x08\x9E\x8D\xD7\xE2\x71\xD1\x3E\x53\xD2\xC6\xEE\x96\xBC", 0x81);
  ...
  strcpy((char *) CHAR_RND_ID, "bc5ffd9299000000");
  ...
  memcpy(AFTER_CRED,
         "\x4B\xBB\xBB\xBB\xBB\xBB\xBB\xBB\xBB\xBB\xBB\xBB\xBB\xBA\xDD\xDD"
             ...
             "\xAA\xAA\xAA\xAA\xAA\xAA\xAA\xAA\xAA\xAA\xAA\xAA\xAA\xAA\xAA\xBC", 0x81);
  ...
  memcpy(CREDENTIALS188,
         "\x00\x00\x01\x04\x00\x00\x00\x01\xA7\xCF\xDF\xF5\xA9\x69\x80\x2C"
             ...
             "\x7F\x49\xF1\x9D\xF5\xBA\xE8\x5D", 0x189);
  ...
  memcpy(xoteg_sec,
         "\xC3\xD3\x81\xF6\x46\xDD\xAA\xBD\xDD\x23\xDA\x29\x52\x49\x11\xC9"
             ...
             "\x27\x84\x3D\xB9\x58\xB5\xD6\xEB\xBC\xE0\xCD\x17\xE3\x47\x19\x99", 0x81);
  ...
  memcpy(skype_pub,
         "\xB8\x50\x6A\xEE\xD8\xED\x30\xFE\x1C\x0E\x67\x74\x87\x4B\x59\x20"
             ...
             "\xCA\x82\x51\xB5\x02\x58\x74\x8D\xBC\x37\xFE\x33\x2E\xDC\x28\x55", 0x101);
  ...
  memset(REMOTE_CHAT_STRING, 0x00, 0x100);

  GOT_REMOTE_MSG_COUNT = 0;

  HEADER_ID_CMD27_CRC = 0;
  HEADER_ID_CMD27_ID = 0;

  HEADER_ID_CMD13R3_ID = 0;
  HEADER_ID_CMD13R4_ID = 0;

  memset(REMOTE_MSG, 0x00, 0x1000);
  REMOTE_MSG_LEN = 0;

  RECV_MANYMSG_BUF = NULL;

  RECV_MANYMSG_BUF_LEN = 0;

  RECV_MANYMSG_00_06_size_total = 0;
  RECV_MANYMSG_00_06_size_recv = 0;
  RECV_MANYMSG_00_06_flag = 0;


  global_unknown_cmd24_time = 0;
  global_unknown_cmd24_time_sec = 0;
  global_unknown_cmd24_blob1b = 0;

  // end of recv specific
  ...
  memcpy(LOCALNODE_VCARD,
         "\xE0\x3E\x31\xAE\x40\x3A\xE0\x12\x00\x75\x03\x25\xC7\xE1\x08\x41\x37\xDF\x19\x9C\x55\xC0\xA8\x01\x4B\xE1\x08",
         0x1B);
  memcpy(REMOTENODE_VCARD,
         "\x70\xE7\xDC\x1C\xE2\x82\x8C\x31\x00\xA8\x3F\x7D\x7D\x9C\x5F\x00\x00\x00\x00\x00\x00\xAC\x1F\xFF\xF9\x9C\x5F",
         0x1B);
...
unsigned int make_setup_prepare() {
  ...
  global_unknown_cmd24_signed_id = 0x21D598C5;
  global_unknown_cmd2A_signed_id = 0x440E8922;

  // 1 megabyte for many msg buffer
  RECV_MANYMSG_BUF = malloc(1 * 1024 * 1024);
  RECV_MANYMSG_BUF_LEN = 0;
...
unsigned int make_tcp_client_sess1_pkt2(char *ip, unsigned short port) {
  ...
  debuglog("Sending second TCP packet\n");
  ...
  if (len <= 0) {
    debuglog("recv timeout\n");
    debuglog("Remote host dropped connection. Check if Credentials expired.\n");
    return len;
  };
