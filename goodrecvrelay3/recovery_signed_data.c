...
int add_cert(char *credentials, char *pubkey) {
	int UIC_CRC;
	int i;
...
int decode_signed_text(char *remote_credentials, int remote_credentials_len, char *remote_signblock, int remote_signblock_len, int need_save_msg){
	u8 outbuf[0x100];
    ...
    enc41_start = 1+0x14+strlen(CHAT_STRING);
	main_unpack(signed_text+enc41_start, signed_text_len-enc41_start);
    ...
    if (need_save_msg) {
        ...
        // extract remote_header_id of msg and save it to db
		debuglog("::::::::::::::Got remote_header_id:\n");
		remote_header_id = 0;
        remote_header_id = get_00_07_blob(signed_text+enc41_start, signed_text_len-enc41_start);
		debuglog("remote_header_id: 0x%08X\n", remote_header_id);

        // extract remote chat_string and save it
		debuglog("::::::::::::::Got REMOTE_MSG pkt\n");
		remote_str_len = get_03_02_blob(signed_text+enc41_start, signed_text_len-enc41_start, remote_str);
        if ((remote_str_len > 0) && (remote_str_len < 64000)) {

            sprintf(REMOTE_MSG+strlen(REMOTE_MSG), "%s\n", remote_str);

            //REMOTE_MSG_LEN = REMOTE_MSG_LEN + strlen(REMOTE_MSG) - 1;

            debuglog("REMOTE_MSG %s\n", REMOTE_MSG);

            // dont need anymore? save only in db
            //save_msg_tofile(remote_str);

            // start of working with db
            // saving new prepared msg to db with our local_header_id.
            // its test message which was not sended
            // REMOTE_NAME -- author
            // 0 -- is_service
            ret = save_message_to_db(remote_header_id, 0x00, remote_header_id, remote_str, REMOTE_NAME, 0);
            if (ret < 0) { return -1; };
    ...
    if (1) {
        ...
        if (data_int > 0) {
            global_unknown_cmd24_time_sec = data_int;
            debuglog("global_unknown_cmd24_time_sec = 0x%08X\n", global_unknown_cmd24_time_sec);
        };
    };
...
//
// not need to save received msg
// because its called from cmd24, cmd2A and for debuglog
//
int recovery_signed_data(char *buf41, int buf41_len) {
	...
	decode_signed_text(remote_credentials, remote_credentials_len, remote_signblock, remote_signblock_len, 0);
		...
		if (CHAT_CMD == 0x2B){
            ...
            for (i=0;i<blobs_count;i++) {
                ...
                if (ret) {
                    ...
                    decode_signed_text(remote_credentials, remote_credentials_len, remote_signblock, remote_signblock_len, 1);
                };
            };
